...
public class FileOperations {
	...
	void writeResult(String result, int key, String operation){
		String extension = this.path.substring(this.path.lastIndexOf('.') + 1, this.path.length());
		String resultPath = this.path.substring(0, this.path.lastIndexOf('.')) + "_" + operation + "." + extension;
		String keyPath = this.path.substring(0, this.path.lastIndexOf('\\')) + "\\" + "key." + extension;
		if(this.addDir){
			resultPath = resultPath.substring(0, resultPath.lastIndexOf('\\')) + this.dirPath + 
