	...
	public void encryptDirectory() {		
		...
		for(final File fileEntry : fpath.listFiles()) {
			if(fileEntry.isDirectory()) {
				...
				Thread t = new Thread(){
					public void run(){
						String extension = fileEntry.getAbsolutePath().substring(fileEntry.getAbsolutePath().lastIndexOf('.') + 1, fileEntry.getAbsolutePath().length());
						if(extension.equals("txt")){
							FileOperations f = new FileOperations(fileEntry.getAbsolutePath(), true, "\\encrypted");
			...
			if(fileEntry.isDirectory()) {
				...
				Thread t = new Thread(){
					public void run(){
						String extension = fileEntry.getAbsolutePath().substring(fileEntry.getAbsolutePath().lastIndexOf('.') + 1, fileEntry.getAbsolutePath().length());
						if(extension.equals("txt")){
							FileOperations f = new FileOperations(fileEntry.getAbsolutePath(), true, "\\decrypted");
							fe.performAlgorithm(f);
						}
					}
				};
